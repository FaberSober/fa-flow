...
- [x] **F-SEC-02 租户、系统字段和子表归属校验**
  ...
  - 实施记录（2026-09-15）：新增租户上下文强制校验；主表 CRUD、流程实例绑定和流程状态关联查询统一追加租户条件；新增数据由服务端写入 `tenant_id`，忽略客户端 `tenant_id`/`flow_instance_id`；子表清空、更新、新增均校验主表外键和租户，更新前增加子表 ID 归属校验；表单主子表写入、删除使用 Spring 事务连接；动态表租户字段统一为字符类型，历史数值型租户字段失败关闭并需先迁移。
...
- [ ] **F-WF-01 流程发布/启动一致性**
  ...
  - 阶段实施记录（2026-09-15）：发布改为读取服务端流程定义，并在同一事务、分布式锁内执行；相同活动引擎部署按模型内容幂等复用，失败时回滚本地状态。启动统一校验本地/引擎启用状态、租户、表单启用状态与归属、`useScope` 和业务关联人；按 `requestId` 生成有界业务幂等键，优先返回活动或历史实例；自定义表单保存、引擎启动和 `flow_instance_id` 绑定共用事务，绑定禁止覆盖其他实例；`retInstanceStart` 已改为复用同一启动命令，前端入口补充并复用 `requestId`。
  - 未完成项：SYSTEM 表单目前仍由业务表单组件先调用独立业务 API 保存，随后才调用通用启动接口，通用模块无法将该业务保存纳入同一事务或自动补偿。因此本项暂不勾选；后续需引入可注册的系统表单命令/补偿适配器，并补充跨资源失败与重试测试后再完成验收。
